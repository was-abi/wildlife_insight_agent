#!/usr/bin/env python3
"""
Wildlife Insight Agent - Production Streamlit Web Interface

Production version with environment variable support for secure API key management.
"""
...
def create_demo_report(species_query: str, species_count: int) -> str:
    """
    Create a demo report when API key is not available.
    """
    return f"""
# Wildlife Insight Report: {species_query.title()}

## 🔍 Search Results
We found **{species_count:,}** records related to "{species_query}" in the GBIF database.

## 📊 Key Findings
- **Species Diversity**: The search revealed various species with "{species_query}" in their names
- **Data Availability**: {species_count:,} records are available for analysis
- **Research Potential**: This data can provide insights into biodiversity patterns

## 🌍 Conservation Insights
Understanding species distribution and occurrence patterns is crucial for:
- **Habitat Protection**: Identifying critical areas for conservation
- **Population Monitoring**: Tracking species abundance over time  
- **Threat Assessment**: Evaluating risks to species survival
- **Research Planning**: Guiding future scientific studies

## 📚 Educational Value
This data helps us learn about:
- Biodiversity patterns across different regions
- Species relationships and taxonomy
- Conservation priorities and needs
- The importance of data collection in science
...
def main():
    ...
    if demo_mode:
        st.warning("""
        🔧 **Demo Mode**: This app is running in demo mode with limited functionality. 
        For full AI-powered analysis, an API key configuration is required.
        """)
    ...
    if analyze_button:
        ...
        try:
            ...
            species_data = fetch_species_data_production(species_query)
            ...
            if demo_mode:
                ...
                try:
                    ...
                    st.error(f"Error in full analysis: {e}")
                    result = create_demo_report(species_query, species_count)
            ...
            if result:
                ...
                with tab3:
                    ...
                    metadata = {
                        ...
                        'Mode': 'Demo Mode' if demo_mode else 'Full AI Analysis',
